...
function stableSort(arr, sortFunction) {
	...
	// Remove pass from items
	for (i = 0; i < length; i++) {
		delete arr[i].ss_i; // stable sort pass
	}
}
...
Pointer.prototype = {
	...
	pinchTranslateDirection: function (horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch) {
		...
		if (outOfBounds) {
			...
			// Set the scale, second index to adapt to the modified touchNow positions
			setScale();
...
Series.prototype = {
	...
	processData: function (force) {
		...
		// If the series data or axes haven't changed, don't go through this. Return false to index
